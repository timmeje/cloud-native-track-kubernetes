...
Create a file named `lab-12-limited-pod.yml` with the following content.
...
```
kubectl apply -f lab-12-limited-pod.yml -n lab-12
...
Create the following file named `lab-12-resource-quota.yml`.
...
```
kubectl apply -f lab-12-resource-quota.yml
```

This will have created a ResourceQuota which on the namespace lab-12. If we try 
...
```
kubectl apply -f lab-12-limited-pod.yml
...
```
kubectl delete resourcequota demo
...
Create a file named `lab-12-limitrange.yml` with the following content:
...
```
kubectl apply -f lab-12-limitrange.yml
...
Error from server (Forbidden): error when creating "lab12-limited-pod.yml": pods "container-info" is forbidden: minimum cpu usage per Container is 100m, but request is 1m
